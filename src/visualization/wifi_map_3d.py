...
def render_wifi_scene(
    ...
    if aps:
        ...
        distances = np.asarray([max(ap.distance_m, 0.01) for ap in aps], dtype=float)
        labels = [
        ...
        for ap, radius in zip(aps, distances):
            sx, sy, sz = _sphere_mesh((ap.x, ap.y, ap.z), radius)
            fig.add_trace(
                go.Surface(
                    x=sx,
                    y=sy,
    ...
    legend_text = (
        ...
        "• Translucent spheres: estimated AP coverage radius from distance model<br>"
